...
**STRICT OPERATIONAL RULES for Phase 1A:**

1.  **Patience is Mandatory**: This is a long-running command (5-15 minutes). Start it in the background and proceed to Phase 1B while it runs.
2.  **Do NOT Restart**: Once started, never run this command again in the same session unless it explicitly exits with a non-zero error code. Slow output, silence, or `No output` are NOT errors — they are normal ZAP behavior.
3.  **No Excessive Probing**: Do NOT ping the host, curl endpoints, or check network connectivity while the scan is running. This wastes time and may interfere with ZAP. Checking `command_status` every 2-3 minutes is sufficient.
4.  **Quiet Monitoring**: If `command_status` shows `No output` or only progress lines like `[SCAN] 45%...`, ZAP is working. Do not be alarmed. Do not restart.
5.  **What a real error looks like**: A real failure exits with a Python traceback (`Traceback (most recent call last):`) or an explicit `[!]` error message, followed by the process terminating. Anything else is progress.
6.  **Forbidden actions while scan is running**: Do not run any `py` command targeting the same host, do not call `bulk-send`, do not read `zap_results.json`. The only allowed action is `command_status` to check progress.
...
**Then check if `tools/idor-check.py` exists.**

- **If exists**: Run it:

    ```
    py tools/idor-check.py \
...
- **If not exists**: Log `[idor-check] Tool not available — candidates queued for manual verification.` and continue. Record `idor_candidates[]` in the final report under "Manual Verification Required".
...
> **Important on scope**: When building each `bulk-send` manifest, include ALL endpoints classified as PROBE_NEEDED or ENDPOINT_OF_INTEREST — not just 3. The limit is 3 *rounds* of bulk-send, not 3 *endpoints*.
...
#### Round 1 — `bulk-send` #1 of 3

Targets: PROBE_NEEDED and high-value ENDPOINT_OF_INTEREST.

For each target:

- Use `site-forms.txt` to confirm exact `param` name and `method`.
- Select payloads **not already in** `zap_reduced.json[*].payloads`.
- Max 3 payloads per endpoint.

Steps:

1. Write `manifest_round1.json`.
2. `$env:PYTHONUTF8=1; py tools/agent-scaner.py bulk-send -m manifest_round1.json -s target_info/session.txt`
3. Note: `response_id`, `status_code`, `ms`, `len` per request.
4. `$env:PYTHONUTF8=1; py tools/agent-scaner.py search --ids <all_ids> -k <keywords>`
5. For each hit → candidate finding (endpoint + param + payload + snippet).

`[bulk-send: 1/3]`

#### Round 2 — `bulk-send` #2 of 3 (if needed)

Remaining PROBE_NEEDED items, or partial hits needing different vectors.
Use `manifest_round2.json`.

`[bulk-send: 2/3]`

#### Round 3 — `bulk-send` #3 of 3 (if needed)

WAF/filter bypass attempts. Encoding variations, comment injection, case alternation.
Edge cases from Round 2 with anomalous `ms`/`len` but no keyword hit.
Use `manifest_round3.json`.
